...
class AnimeSearch(Resource):
    def get(self, name):
        ...
        result = requests.get(
            'https://www.goload.one//search.html?keyword=' + name)
...
class AnimeDetail(Resource):
    def get(self, animeId):
        URL = 'https://www.goload.one/category/' + animeId
...
class AnimeGenre(Resource):
    def get(self, genre, page_number):
        ...
        anime_results = []
        result = requests.get(
            'https://www.goload.one/genre/' + genre + '?page=' + page_number)
